          ...
          <div class="shadow-md sm:px-3 sm:rounded-md sm:overflow-hidden">
            <div class="bg-white py-6 px-4 space-y-6 sm:p-6">
              <div class="py-6">
                ...
                <p class="mt-1 text-sm text-gray-500">You can update the user details.</p>
              </div>
...
async function fetchSpecificUser(id) {
    ...
    try{
        const response = await fetchAPI(`https://fakestoreapi.com/users/${id}`);
        user.value = response
...
async function UpdateNewUser() {
    const {onFailure, onSuccess} = useShowSnackbar()
    try{
        const response = await fetchAPI(`https://fakestoreapi.com/users/${id}`, 'PUT', user);
